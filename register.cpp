...
//                "is_online INTEGER DEFAULT 0)");//此处表明email不为空，则后面插入管理员时不能为空，随便写一个替代即可
    ...
    if (!createTableQuery.exec(sql)) {
            qDebug() << "Create users table error:" << createTableQuery.lastError();
        }else{
        qDebug()<<"Create users table sucess.";
    }
////////////////////////////////////////////////////////////////////////
    // 检查并添加管理员账户
        QString checkAdminSql = "SELECT * FROM users WHERE xuehao = 'admin'";
        ...
        if (!checkAdminQuery.exec(checkAdminSql)) {
            ...
            // 管理员账户不存在，插入管理员账户
            QString insertAdminSql = "INSERT INTO users (xuehao, password, email,create_time) VALUES ('admin', 'admin123', 'admin@example.com','2024.7.10')";
...
/////////////////////////////////////////////否则不发送验证码，不管验证码也能注册
        ...
        QString sql=QString("INSERT INTO users(xuehao,password,email,create_time) VALUES (?,?,?,?)");
